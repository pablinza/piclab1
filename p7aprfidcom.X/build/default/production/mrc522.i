...
uint8_t MRCToCard(uint8_t cmd, uint8_t *src, uint8_t slen, uint8_t *dst, unsigned *dlen)
{
    ...
    MRCWriteReg(0x02, irqEn | 0x80);
    MRCClearBit(0x04, 0x80);
    MRCSetBit(0x0A, 0x80);
    MRCWriteReg(0x01, 0x00);
    for(i=0; i < slen; i++)
        MRCWriteReg( 0x09, src[i] );
    MRCWriteReg(0x01, cmd);
    if(cmd == 0x0C)
       MRCSetBit(0x0D, 0x80);
    ...
    MRCClearBit(0x0D, 0x80);
    if (i != 0)
    {
        if( !(MRCReadReg(0x06) & 0x1B ))
        {
            status = 0;
            if ( n & irqEn & 0x01 )
                status = 1;
...
{
    uint8_t i, n;
    MRCClearBit(0x05, 0x04);
    MRCSetBit(0x0A, 0x80);
    for ( i = 0; i < len; i++ )
        MRCWriteReg(0x09, *src++);
    MRCWriteReg(0x01, 0x03);
    i = 0xFF;
    do
    {
        n = MRCReadReg(0x05);
    ...
    while(i && !(n & 0x04));
    dst[0] = MRCReadReg(0x22);
    dst[1] = MRCReadReg(0x21);
}
...
uint8_t MRCRead(uint8_t baddr, uint8_t *dst)
{
    ...
    status = MRCToCard(0x0C, dst, 4, dst, &len);
    if ((status != 0) || (len != 0x90)) { status = 2; }
    return status;
}
...
uint8_t MRCWrite(uint8_t baddr, uint8_t *src)
{
 ...
 uint8_t buff[18];
    ...
    status = MRCToCard(0x0C, buff, 4, buff, &recvbits);
    ...
    if (status == 0)
    {
        ...
        status = MRCToCard(0x0C, buff, 18, buff, &recvbits);
        if ((status != 0) || (recvbits != 4) || ((buff[0] & 0x0F) != 0x0A))
          status = 2;
    }
    return status;
...
void MRCEncodeValue(int32_t value, uint8_t baddr, uint8_t *dst)
{
    ...
    dst[5] = ~res;
    dst[9] = res;
    res = (uint8_t) (value>>16);
    dst[2] = res;
    dst[6] = ~res;
    dst[10] = res;
    res = (uint8_t) (value>>24);
    dst[3] = res;
    dst[7] = ~res;
    dst[11] = res;
    dst[12] = baddr;
...
uint8_t MRCUpdateValue(uint8_t baddr, uint8_t cmd, uint8_t step)
{
 ...
 uint8_t buff[8];
    ...
    if (status == 0)
    {
        ...
        MRCToCard(0x0C, buff, 6, buff, &recvbits);
        buff[0] = 0xB0;
        buff[1] = baddr;
        MRCRunCRC(buff, 2, &buff[2]);
        status = MRCToCard(0x0C, buff, 4, buff, &recvbits);
...
void MRCInit(void)
{
    ...
    TRISAbits.TRISA5 = 0;
    __nop();
    MRCWriteReg(0x01, 0x0F);
    MRCWriteReg(0x2A, 0x8D);
    MRCWriteReg(0x2B, 0x3E);
    MRCWriteReg(0x2D, 30);
    MRCWriteReg(0x2C, 0);
    MRCWriteReg(0x15, 0x40);
    MRCWriteReg(0x11, 0x3D);
    MRCWriteReg(0x26, 0x70);
    MRCClearBit(0x14, 0x03) ;
    MRCSetBit(0x14, 0x03);
}
# 318 "mrc522.c"
uint8_t MRCRequest(uint8_t cmd, uint8_t *dst)
{
    uint8_t status;
    unsigned lenbits;
    MRCWriteReg(0x0D, 0x07);
    dst[0] = cmd;
    status = MRCToCard(0x0C, dst, 1, dst, &lenbits);
    if ((status != 0) || (lenbits != 0x10))
        status = 2;
    return status;
}
...
uint8_t MRCAntiColl(uint8_t *sernum)
{
    ...
    unsigned unlen;
    MRCWriteReg(0x0D, 0x00);
    sernum[0] = 0x93;
    sernum[1] = 0x20;
    MRCClearBit(0x08, 0x08);
    status = MRCToCard(0x0C, sernum, 2, sernum, &unlen);
    if(status == 0)
    {
        for(i=0; i < 4; i++)
...
uint8_t MRCSelectTag(uint8_t *sernum)
{
    ...
    status = MRCToCard(0x0C, buffer, 9, buffer, &recvbits);
...
uint8_t MRCAuth(uint8_t authmode, uint8_t baddr, uint8_t *key, uint8_t *src)
{
 ...
 uint8_t buff[12];
    ...
    for (i=0; i<4; i++)
        buff[i+8] = *(src+i);
    status = MRCToCard(0x0E, buff, 12, buff, &recvbits);
    if ((status != 0) || (!(MRCReadReg(0x08)&0x08)))
        status = 2;
    return status;
}
...
void MRCHalt(void)
{
    ...
    buff[1] = 0;
    MRCRunCRC( buff, 2, &buff[2] );
    MRCClearBit( 0x08, 0x80 );
    MRCToCard( 0x0C, buff, 4, buff, &len );
    MRCClearBit( 0x08, 0x08 );
}
