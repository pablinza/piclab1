...
void taskTIM(void)
{
    ...
    {
        case 0:
            uptime = (uint24_t) eeprom_read(0);
            uptime <<= 8;
            uptime |= (uint24_t) eeprom_read(1);
            uptime <<= 8;
            uptime |= (uint24_t) eeprom_read(2);
            dotime = (uint24_t) eeprom_read(3);
            dotime <<= 8;
            dotime |= (uint24_t) eeprom_read(4);
            dotime <<= 8;
            dotime |= (uint24_t) eeprom_read(5);
            state = 1;
            break;
        case 1:
            RTCGetdt();
